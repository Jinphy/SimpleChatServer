...
public class Test {
    ...
    public static <U> void m(String json) {
        Type type = new TypeToken<Response<List<U>>>() {}.getType();
        Response<List<User>> response = GsonUtils.toBean(json, type);

        for (User user : response.getData()) {
            System.out.println("name: " + user.getName() + ", account: " + user.getAccount());
        }
    }
